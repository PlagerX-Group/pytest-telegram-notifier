...
class TelegramManager:
    ...
    def pytest_telegram_notify_message_additional_fields(self, config: Config) -> dict:
        return {}

    @pytest.hookimpl
    def pytest_telegram_notify_message_template(self, additional_fields: dict) -> str:
    ...
    def pytest_sessionfinish(self, session: Session):
        additional_fields = self._config.hook.pytest_telegram_notify_message_additional_fields(config=self._config)[0]
        template = self._config.hook.pytest_telegram_notify_message_template(additional_fields=additional_fields)[0]
